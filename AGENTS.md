...
All git commits must follow the repository commit style.
...
Use pull requests for app changes, even when working alone.
...
* safer changes
* easier rollback
* agents can explain changes
* discussions stay attached to code
* more trusted and open-source ready

Branch names should be short, lowercase, and scoped:
...
PR titles should match the commit identity style:

```text
[chat] preserve unsent draft
...
For all other work, create a feature branch and open a PR before merging.
...
Only use Gradle tasks that currently exist in the project. Do not add
placeholder CI steps for ktlint, detekt, dependency review, Firebase validation,
or any other tool until the plugin, task, or integration is actually configured.
...
Leave small review comments occasionally when there is reasoning worth preserving.

Good review comments explain architecture, tradeoffs, edge cases, or future cleanup:
